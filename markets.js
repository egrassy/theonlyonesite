...
function draw() {
  ...
  ctx.textAlign = 'center'; ctx.fillStyle = '#93899f'; const labels = Math.min(6, points.length); for (let i = 0; i < labels; i++) { const index = Math.round(i * (points.length - 1) / Math.max(1, labels - 1)); ctx.fillText(formatDate(points[index].time), x(index), rect.height - 10); }
  if (hover >= 0 && points[hover]) { const xx = x(hover); ctx.strokeStyle = 'rgba(255,255,255,.32)'; ctx.beginPath(); ctx.moveTo(xx, pad.top); ctx.lineTo(xx, rect.height - pad.bottom); ctx.stroke(); ctx.fillStyle = '#e7deff'; ctx.fillText(formatDate(points[hover].time), xx, rect.height - 10); }
}
function fillGaps(rows) {
  if (!rows.length) return []; const result = [], step = STEP[timeframe]; let last = null, at = candleStart(rows[0].time);
  for (const row of rows) { const target = candleStart(row.time); while (last && at < target) { result.push({ time: at, open: last.close, high: last.close, low: last.close, close: last.close, flat: true }); at += step; } result.push({ ...row, time: target, flat: row.open === row.close && row.high === row.low }); last = row; at = target + step; }
  return result;
}
async function load() {
  ...
  try {
    ...
    const input = { json: { poolAddress: POOLS[pair].address, from, to, quote: 'uluna', interval: INTERVAL[timeframe] } };
    ...
    candles = fillGaps(rows); shift = 0; hover = -1; const latest = rows.at(-1);
