...
function formatVolume(value) { const n = Number(value || 0); if (n >= 1e12) return (n / 1e12).toFixed(2) + 'T'; if (n >= 1e9) return (n / 1e9).toFixed(2) + 'B'; if (n >= 1e6) return (n / 1e6).toFixed(2) + 'M'; if (n >= 1e3) return (n / 1e3).toFixed(2) + 'K'; return n.toFixed(0); }
...
function setReadout(points, index) { const node = document.querySelector('#ohlc-readout'), point = points?.[index]; if (!point) { node.textContent = 'Hover a candle: O — · H — · L — · C — · Δ — · Vol —'; return; } const change = changeFromPrevious(points, index); node.textContent = `${formatDate(point.time)} · O ${point.open.toExponential(3)} · H ${point.high.toExponential(3)} · L ${point.low.toExponential(3)} · C ${point.close.toExponential(3)} · Δ ${change >= 0 ? '+' : ''}${change.toFixed(2)}% · Vol ${formatVolume(point.volume)}`; }
function draw() {
